...
def should_fail(findings: Iterable[dict[str, Any]], fail_on: str) -> bool:
    ...
    return any(SEVERITY_RANK.get(str(item.get("severity", "info")), 0) >= threshold for item in findings)
    ...
    for finding in findings:
        ...
        rules.setdefault(
            rule_id,
            {
                ...
                "help": {"text": str(finding.get("hint") or finding.get("message") or rule_id)},
            },
        )

        location = str(finding.get("location") or scanned_path)
        file_location = location.split(":", 1)[0]
        region: dict[str, Any] = {}
        line = finding.get("line")
        if isinstance(line, int) and line > 0:
            region["startLine"] = line

        physical: dict[str, Any] = {"artifactLocation": {"uri": file_location}}
        if region:
            physical["region"] = region

        result: dict[str, Any] = {
            "ruleId": rule_id,
            "level": {"error": "error", "warning": "warning", "info": "note"}.get(severity, "note"),
            "message": {"text": str(finding.get("message", ""))},
            "locations": [{"physicalLocation": physical}],
        }
        if finding.get("hint"):
            result["properties"] = {"hint": str(finding["hint"]), "mcpGuardSeverity": severity}
